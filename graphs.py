...
def maps(df, carburant, prixmax, maposition, list_services, rayon): 
    # Affichage pour voir le nombre de lignes
    print('Il y a ' + str(len(df)) + ' enregistrements dans les données avant de supprimer.')
    print()
    t1 = datetime.datetime.now()
    ...
    df = df.query('prix_nom =="' + carburant + '" & prix_valeur < '+ str(prixmax))
    print(str(prixmax))
    ...
    for index, location_info in df.iterrows():
        ...
        folium.Marker(
            [float(x[0]),float(x[1])], 
            popup=location_info["services_service"],#location_info["adresse"]+" "+location_info["ville"],
            icon=gaspump,
        ).add_to(world_map)
